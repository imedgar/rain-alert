...
func (api *WeatherAPI) GetNextHourForecast() error {
	...
	th, err := api.getThresholds(api.Db)
	if err != nil {
		return err
	}
	...
	if hour.ChanceOfRain < api.Thresholds["drizzleThreshold"] {
		log.Printf("Chance of rain (%d%%) too low, not notifying.\n", hour.ChanceOfRain)
		return nil
	}

	notify, err := api.shouldNotify(api.Db)
	if err != nil {
		return err
	}
	if !notify {
	...
	if err := api.recordNotification(api.Db, hour.ChanceOfRain); err != nil {
		return err
	}
...
func (api *WeatherAPI) shouldNotify(db *sql.DB) (bool, error) {
	...
	if state > api.Thresholds["rainBeforeThreshold"] {
		return false, nil
	}
...
func (api *WeatherAPI) getThresholds(db *sql.DB) (map[string]int, error) {
